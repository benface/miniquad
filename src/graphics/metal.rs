...
impl RenderingBackend for MetalContext {
    ...
    fn apply_scissor_rect(&mut self, x: i32, y: i32, w: i32, h: i32) {
        ...
        // clamp into `[0, pass_size]` on both axes: during rotation
        // the caller's view of the screen size lags MTKView's
        // drawable by a frame, and an unclamped scissor would trip
        // Metal validation (`x + w > pass width`, `y + h > pass
        // height`) in debug builds.
    ...
    fn new_pipeline(
        ...
        unsafe {
            ...
            // Match the view's depth/stencil format — MTKView
            // defaults to `Invalid` when none is configured, and
            // the pipeline must match or validation fails.
