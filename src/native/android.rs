...
/// Query the activity's display density (the `DisplayMetrics.density`
/// field, where `1.0` is mdpi = 160 DPI, `2.0` is xhdpi = 320 DPI,
/// etc.). Used to populate `native_display().dpi_scale` so higher-level
/// APIs (e.g. macroquad's `screen_width()` which divides by `dpi_scale`)
/// can convert framebuffer pixels to density-independent units.
